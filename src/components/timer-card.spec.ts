...
describe('Timer Card', () =>{
 it('Shows title', () => {
    const w = shallowMount(TimerCard, { props:  { duration: 10, zone: TrainingZone.WarmUp }});
    expect(w.find('[data-test-id="timer-title"]').text()).toBe('10 Minute Timer - Warm Up');
 });
 ...
 it('Shows zone information', () => {   
   const w = shallowMount(TimerCard, { props:  { duration: 3, zone: TrainingZone.MSS}});
   expect(w.find('[data-test-id="timer-title"]').text()).toBe('3 Minute Timer - Maximal Steady State (MSS)');
   expect(w.find('[data-test-id="heart-rate"]').text()).toBe('Heart Rate: 165-175 BPM');
   expect(w.find('[data-test-id="perceived-exertion"]').text()).toBe('Perceived Exertion: 5-8');
